  ...
  return (
    <button
      ...
      className={`
        ...
        w-full h-full min-h-[64px] sm:min-h-[90px] p-1.5 sm:p-2 rounded-xl border bg-surface
    ...
    >
      {/* Oben: Datum */}
      <span className={`text-base sm:text-lg font-bold ${day.termine > 0 ? 'text-ivory' : 'text-muted'}`}>
        {day.date.getDate()}
      </span>
      ...
      {day.termine > 0 ? (
        <span className={`
          self-start px-1.5 sm:px-2 py-0.5 sm:py-1 text-xs font-bold rounded-full shadow-sm border
          ${day.termine >= 9
        ...
        `}>
          {/* Auf Mobile nur die Zahl, ab sm der volle Text */}
          <span className="sm:hidden">{day.termine}</span>
          <span className="hidden sm:inline">{terminText}</span>
        </span>
      ) : (
